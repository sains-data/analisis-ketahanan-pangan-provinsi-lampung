"""
Silver Layer ETL: Social Economic Data Cleaning Script

This script processes social economic data from the bronze layer, performs cleaning
...
def load_bronze_data(spark):
    """Load social economic data from bronze layer"""
    logger.info("Loading social economic data from bronze layer")
    
    input_path = f"{str(HDFS_CONFIG['data_paths']['bronze'])}/sosial_ekonomi.csv"
    
    df = read_csv_from_hdfs(
        spark,
        input_path,
        separator=";",  # sosial_ekonomi.csv uses semicolon separator
...
def validate_input_data(df):
    ...
    expected_columns = ["Kabupaten/Kota", "Tahun", "Jumlah_Penduduk", "Persentase_miskin", "pengeluaran_per_kapita"]
...
def remove_empty_records(df):
    """Remove records with empty semicolon patterns (;;;;)"""
    return df.filter(
        col("Tahun").isNotNull() 
        & (trim(col("Tahun")) != "")
        & col("Jumlah_Penduduk").isNotNull()
        & (trim(col("Jumlah_Penduduk")) != "")
        & col("Persentase_miskin").isNotNull()
        & (trim(col("Persentase_miskin")) != "")
    )
...
def normalize_column_names(df):
    ...
    return (df
            .withColumnRenamed("Kabupaten/Kota", "kabupaten_kota")
            .withColumnRenamed("Tahun", "tahun")
            .withColumnRenamed("Jumlah_Penduduk", "jumlah_penduduk")
            .withColumnRenamed("Persentase_miskin", "tingkat_kemiskinan")
            .withColumnRenamed("pengeluaran_per_kapita", "ipkm"))
...
def clean_numeric_columns(df):
    ...
    return (df
            ...
            .withColumn("tingkat_kemiskinan", col("tingkat_kemiskinan").cast(DoubleType()))
            .withColumn("ipkm", col("ipkm").cast(DoubleType())))
...
def filter_valid_records(df):
    ...
    return df.filter(
        ...
        & col("jumlah_penduduk").isNotNull()
        & (col("jumlah_penduduk") > 0)
        & col("tingkat_kemiskinan").isNotNull()
        & (col("tingkat_kemiskinan") >= 0)
        & (col("tingkat_kemiskinan") <= 100)
        & col("ipkm").isNotNull()
        & (col("ipkm") > 0)
        & (col("tahun") >= 2000) 
        & (col("tahun") <= 2030)
...
def clean_and_transform(df):
    ...
    df_cleaned = (df
                  .transform(remove_empty_records)
                  ...
                  .dropDuplicates()
                  .select("kabupaten_kota", "jumlah_penduduk", "tingkat_kemiskinan", 
                         "ipkm", "tahun"))
...
def validate_output_data(df):
    ...
    required_columns = ["kabupaten_kota", "jumlah_penduduk", "tingkat_kemiskinan", "tahun"]
    ...
    high_poverty = df.filter(col("tingkat_kemiskinan") > 30).count()
    low_population = df.filter(col("jumlah_penduduk") < 100000).count()
    
    logger.info(f"Records with poverty rate > 30%: {high_poverty}")
    logger.info(f"Records with population < 100,000: {low_population}")

    # Check for unrealistic values
    extreme_ipkm = df.filter(col("ipkm") > 5000000).count()
    if extreme_ipkm > 0:
        logger.info(f"Found {extreme_ipkm} records with IPKM > 5,000,000")
...
def save_to_silver(spark, df):
    ...
    output_path = f"{str(HDFS_CONFIG['data_paths']['silver'])}/sosial_ekonomi"
    database = str(HIVE_CONFIG["databases"]["silver"])
    table_name = "sosial_ekonomi"
...
def run_pipeline():
    ...
    spark = None
    try:
        logger.info("Starting social economic data cleaning pipeline")

        # Initialize Spark session
        spark = SparkSessionManager.get_session("CleanSosialEkonomiData")
        ...
        logger.info("Social economic data cleaning pipeline completed successfully")
